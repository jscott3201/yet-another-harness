...
wire protocol for Node/CPython worker processes is specified, generated, and
fixture-pinned on the host side. A supervised process driver spawns,
authenticates, and reaps workers over an inherited fd and publishes an exact
activation endpoint after negotiation. Registered worker-to-host methods run
off-pump behind bounded queues and cooperative cancellation. Its pump binds the
portable text capability from the activation's exact grant snapshot to
session-local handles and releases them through the protocol control frame. A
private ESM TypeScript package now implements incremental framing, strict
directional JSON/schema admission, and worker-message encoding. It has no
handshake/session, application API, transport, or authored worker and does not
implement the host endpoint surface. The repository does not yet
contain a package loader, agent loop, sandbox, daemon, or client. Guest resource
limits exist only inside the Wasmtime driver, and they bound what a guest can
cost, not what it can reach. No cross-runtime equivalence is proven.
...
| Activation-scoped capability broker | Revision-bound requested subsets, exact typed registrations, weak mediated handles, provider/activation ABA fencing, and synchronous call drain before cleanup | One example-only local greeting round trip passes; the Wasm driver carries the portable text contract through its resource table, and the process driver carries the same exact grant through its pump-owned handle table; no policy engine, durable attempt scope, broader production capability family, or hostile-code sandbox |
...
| Worker process wire protocol | Sans-io framed strict-JSON protocol v1 for future Node/CPython workers: length-prefixed frames refused from the prefix alone, duplicate-member, unsafe-integer, and field-bound rejection mirrored by the generated schemas, hello/accept negotiation that fails closed on unknown required features and carries required-only features into the set in force, per-direction calls with never-reused ids and one terminal each — a served artifact pull-read retires its id like every other terminal — frame-counted stream credit split into lossless and lossy classes and widenable from either side up to an announced ceiling, host-enforced deadlines, byte and field bounds that also refuse the host application's own outbound frames, artifact spill behind digest-carrying offers with bounded hex pull-reads, and explicit acknowledged handle release in both directions, capability and artifact handles counted against one live-handle ceiling, with host-side reclamation on every path a release frame cannot travel — err and cancelled terminals, goodbye, disconnect, and fatal fault | One hundred forty-nine deterministic tests drive the host side — one hundred thirteen session fixtures against a scripted byte-level peer, plus a byte-boundary property suite (every prefix and payload split, fragmentation-invariant classification, terminal suffix-insensitive poison, retained bytes and allocation bounded — the allocation grows only with delivered bytes, the high-water is kept for reuse, and a poison releases it — strict-JSON boundary values, escaped-name duplicates, the parser's real recursion limit) and a class-prefixed shared corpus whose inventory the stable gate verifies — including the loss split (goodbye settles in-flight work cancelled, a bare disconnect settles it outcome-unknown with reconciliation required), refusals that echo nothing the worker sent, and paired ceiling cases refused at the bound and admitted below it; three cargo-fuzz targets (incremental decode, strict JSON admission, admitted-frame round trip) attack the byte boundary under arbitrary bytes and chunkings in an isolated nightly-only package, where thirty-second smoke campaigns ran 2.8M, 3.7M, and 1.2M executions under AddressSanitizer with one survivor: a one-ULP float-parsing divergence from the correctly rounded JavaScript and CPython decoders, fixed by the workspace-wide `float_roundtrip` feature and pinned by f64 bit patterns cross-derived from Node's `JSON.parse` and CPython's `json`, which agree with each other and with the Rust parser on every pinned literal; a three-lens adversarial review of the first cut found and closed a served-read id-reuse hole, an artifact exemption from the handle ceiling, and two missing API halves (host credit grants, host-initiated release), and three further passes closed the outbound half — field bounds and an I-JSON integer mirror now bind the host application's own frames, a raw-token scan refuses integer literals the parser would round to floats, a spilled reply must exactly describe an artifact the session holds and ride the call it was minted for, only worker-offered handle ids are releasable and none is ever offered twice, the generated TypeScript admits the null a budget-free call actually carries, release-acks are matched against the kind the release named, a release racing a reclaiming terminal is acked instead of fatal, and served reads clear the same admission ceiling as any call — each fix fixture-pinned; session sequencing, credit, handles, and lifetime correlation are now pinned by an independent reference model — authored from this document, never from the session code — compared against production after every step of twenty-five hundred seeded generated traces plus named boundary cases, with every divergence promoted to a replayable JSON trace in the stable gate, and a retired-operation budget bounds the no-reuse correlation memory: at the budget new admissions are refused without spending an id while already-admitted work completes, the overshoot bounded by the in-flight and live-handle ceilings; a fourth fuzz target drives arbitrary bounded action traces through the live session under the same ceilings; worker-process pumping and long-duration campaigns remain outside the session-level evidence; generated JSON Schema and TypeScript are checked in and gate-checked; the process driver row below supplies the transport, while no worker SDK exists, the host bridge binds the portable text capability only, and worker-side SDK conformance has no implementation to run against |
| Worker process driver | Supervised one-process-per-activation driver over the [wire protocol](plugin-worker-protocol.md): fd-3 socketpair bootstrap where the inherited descriptor is the whole credential (no token in argv, environment cleared to a PATH allowlist, every descriptor above the channel marked close-on-exec so even ambient non-cloexec descriptors cannot ride in), stdout/stderr retained as bounded diagnostics, a byte pump whose reads, writes, deadline ticks, and child-exit watch are independent select arms with the outbound bytes capped, health snapshots naming why a session ended, and an unconditional reclaim path — goodbye, grace window, SIGKILL to the worker's whole process group, reap | Passes all five portable driver-lifecycle conformance cases against real child processes, and a scripted fake worker pins the supervision guarantees end to end: a crash poisons its activation and a fresh activation on the same driver serves (restart is a fresh process, never a resume), a mid-call disconnect settles in-flight work outcome-unknown with reconciliation required while a worker goodbye racing its own exit still settles it cancelled without reconciliation, a cancelled call terminates with the cancelled outcome, an expired deadline settles locally and the late answer is observed arriving (the worker reports answering the retired call) and tolerated, a worker that stops reading stalls neither the deadline clock nor the forced kill and is declared dead at the outbound buffer cap rather than buffered without bound, a mute worker fails its start at the handshake clock, a missing worker program fails by name as a spawn failure rather than masquerading as a protocol disconnect, a worker's death is seen even when a descendant it spawned holds the channel open and that descendant provably dies with the activation's group sweep, an abandoned activation reaps its own worker once the last driver handle drops, deactivation settles work the worker still holds rather than dropping its waiter, and the spawned process provably carries only PATH in its environment and only stdio plus the channel in its descriptor table even with a deliberately leaked non-cloexec descriptor armed in the host; the pathname-socket peer-credential lane is deliberately deferred, restart policy stays with the composition, and the exact capability context in the start permit feeds the portable text routes in the pump. No other capability family or worker SDK is bound. A hostile-pressure suite drives the real socketpair under load: an inbound call flood dies at the in-flight ceiling without wedging the deadline clock, a caller-side command flood meets the bounded command channel with named rejections and no lost admitted call, partial prefix and partial payload at end-of-input close as named faults, a goodbye trickled a byte per millisecond still classifies as a goodbye, diagnostics written just before death are retained, and deactivation under combined pressure reclaims the process group with every admitted call settled exactly once — waiter, channel, and buffer gauges returning to expected values while the activation is live, and the gauges ending with the pump. The pump's command path is bounded by construction: call and cancel requests queue only up to a configured slot count and rejection is observable, while deactivation rides a dedicated queue-free signal no flood can delay or drop. |
| Production process application endpoint | Exact-activation endpoint published after hello/accept and synchronously closed to admission when scope cancellation or deactivation begins; bounded host calls and streams, typed loss, verified worker-spilled reads, acknowledged worker-handle release, and immutable named worker-to-host methods whose callbacks run off-pump under shared queue/concurrency bounds | Real-process tests cover unavailable/live/closing/closed states, stale activation replacement, call and stream flow, dropped waiters, deadline/goodbye/disconnect/fatal distinctions, spill size/media/chunk/hex/digest checks, release acknowledgement and loss, unknown and duplicate methods, saturation, callback panic, cancellation before and during dispatch, and worker reap while a synchronous callback is still returning. Callback cancellation is cooperative: it retires the call path but does not interrupt trusted synchronous code. Spill bytes are activation-local protocol storage, not the durable artifact service. |
| Process text-capability bridge | Protocol-owned acquire and invoke methods resolve the activation permit's immutable text grant, share the application dispatch queue and provider bound, mint through the sole session-mutating pump, and keep broker handles in a pump-owned table synchronized by exact release and reclamation IDs | Real fake-worker cases cover acquire/invoke/release, grant refusal and mismatch, provider replacement fencing, stop and cancellation races, provider panic and bounded failure text, output exhaustion without artifact spill, forged and released handles, shared handle ceilings, release ordering, every reclaim path, sibling isolation, and zero terminal gauges. This is host/process semantic evidence, not a Node or CPython SDK. |
| TypeScript worker wire codec | Private ESM source package with bounded incremental framing, fatal UTF-8 and duplicate/number-preserving JSON admission, strict Ajv 2020 validation of the separate Rust-generated host and worker schemas, generated runtime limits, and outbound rejection where JavaScript JSON would mutate a value | Node 26.7.0 tests reuse the Rust raw-byte corpus and cover directional/schema/encoding boundaries; no handshake/session, Promise/AsyncIterable/AbortSignal API, artifact or handle lifecycle, fd-3 runtime, authored worker, publication, sandbox, Node 24/26 matrix, or full semantic conformance |
| Python worker SDK | The [wire contract](plugin-worker-protocol.md), process driver, and host endpoint exist; the fake worker is a test fixture, not an SDK | Not started beyond the host side |
...
production capability families and execution backends beyond the portable text
bridge, plugin-provided services, worker SDK bindings, durable attempts, and
actual multi-runtime or guest-semantic conformance are not implemented; the WIT
and process bindings exist for the portable text contract only. See
...
- Plugin package loading, verification, admission, installation, updates,
  complete process worker SDKs, production capability families, plugin service
  contributions, policy-derived grants, complete language SDKs,
  guest-semantic corpus, or demonstrated cross-runtime equivalence.
...
- Durable artifact storage. The process endpoint pull-reads and verifies
  worker-spilled results held for one activation, then explicitly releases the
  worker handle. Those ephemeral bytes are not persisted or recovered.
