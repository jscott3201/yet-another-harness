...
| Worker process driver | Supervised one-process-per-activation driver over the [wire protocol](plugin-worker-protocol.md): fd-3 socketpair bootstrap where the inherited descriptor is the whole credential (no token in argv, environment cleared to a PATH allowlist), stdout/stderr retained as bounded diagnostics, a byte pump bridging the sans-io session to the socket with deadline ticks, health snapshots naming why a session ended, and an unconditional reclaim path — goodbye, grace window, SIGKILL, reap | Passes all five portable driver-lifecycle conformance cases against real child processes, and a scripted fake worker pins the acceptance triad end to end: a crash poisons its activation and a fresh activation on the same driver serves (restart is a fresh process, never a resume), a mid-call disconnect settles in-flight work outcome-unknown with reconciliation required, a cancelled call still terminates with the cancelled outcome, an expired deadline settles locally and tolerates the late answer, and the spawned environment provably carries nothing but PATH; the pathname-socket peer-credential lane is deliberately deferred, restart policy stays with the composition, and the capability context in the start permit is accepted but unused — no worker handle is minted from a real grant yet |
