...
fixture-pinned on the host side, and a supervised process driver now spawns,
authenticates, and reaps workers over an inherited fd — with no worker SDK
behind it yet. The repository does not yet contain a general callback host, package
...
| Activation-scoped capability broker | Revision-bound requested subsets, exact typed registrations, weak mediated handles, provider/activation ABA fencing, and synchronous call drain before cleanup | One example-only local greeting round trip passes, and the Wasm driver carries handles for the portable text contract into guests through its store's resource table; no policy engine, durable attempt scope, production capability family, or hostile-code sandbox, and the worker wire protocol's capability-handle encoding is now bound to this broker through the process driver's dispatcher, with the portable text contract as the first served family |
