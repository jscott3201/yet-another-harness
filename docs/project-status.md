...
| Worker process driver | Supervised one-process-per-activation driver over the [wire protocol](plugin-worker-protocol.md): fd-3 socketpair bootstrap where the inherited descriptor is the whole credential (no token in argv, environment cleared to a PATH allowlist), stdout/stderr retained as bounded diagnostics, a byte pump whose reads, writes, deadline ticks, and child-exit watch are independent select arms, health snapshots naming why a session ended, and an unconditional reclaim path — goodbye, grace window, SIGKILL to the worker's whole process group, reap | Passes all five portable driver-lifecycle conformance cases against real child processes, and a scripted fake worker pins the supervision guarantees end to end: a crash poisons its activation and a fresh activation on the same driver serves (restart is a fresh process, never a resume), a mid-call disconnect settles in-flight work outcome-unknown with reconciliation required, a cancelled call still terminates with the cancelled outcome, an expired deadline settles locally and the late answer is observed arriving and tolerated, a worker that stops reading stalls neither the deadline clock nor the forced kill, a mute worker fails its start at the handshake clock, a worker's death is seen even when a descendant it spawned holds the channel open (and the descendant dies with the activation's group sweep), an abandoned activation reaps its own worker once the last driver handle drops, deactivation settles work the worker still holds rather than dropping its waiter, and the spawned process provably carries only PATH in its environment and only stdio plus the channel in its descriptor table; the pathname-socket peer-credential lane is deliberately deferred, restart policy stays with the composition, and the capability context in the start permit is accepted but unused — no worker handle is minted from a real grant yet |
