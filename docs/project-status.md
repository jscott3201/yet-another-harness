...
session-local handles and releases them through the protocol control frame. No
worker SDK implements this host surface yet. The repository does not yet
...
| Node/TypeScript and Python worker SDKs | The [wire contract](plugin-worker-protocol.md), process driver, and host endpoint exist; the fake worker is a test fixture, not an SDK | Not started beyond the host side |
...
- Plugin package loading, verification, admission, installation, updates,
  process worker SDKs, production capability families, plugin service
