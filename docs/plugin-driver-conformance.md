...
| `driver.pending-start-cancellation` | Start stays pending across dropped and resumed waiters; removal cancels before the stored future is dropped; one deactivation releases the partial fixture resource. A driver may return `Pending` before it has acquired anything — one that runs acquisition on a stack it can yield from sometimes does — so the case polls to acquisition under a bound rather than requiring the first poll to reach it; a start that spends the whole bound without acquiring fails the case and is reported as that, not as a retention failure. |
