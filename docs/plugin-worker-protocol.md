...
the kill path is not implemented; neither is any worker-side SDK. Sixty-two
deterministic fixtures in `crates/yah-plugin-ipc/tests/` drive the host side
of every rule below with a scripted byte-level peer.
...
4 KiB) before any payload byte is buffered; a zero-length or oversize
...
terminals `accept` and `refuse`. Control frames (handshake, credit, cancel,
release, goodbye, stream-open) are bounded at 16 KiB; a call payload at
256 KiB; an inline result at 64 KiB; a stream data frame at 64 KiB.
...
call is refused on the sending side and fatal on the receiving side.
...
of what was dropped. `credit` frames widen the window up to a 1024-frame
ceiling. A consumer can `cancel` with the stream target to unsubscribe:
...
continues: an oversize payload, an over-long method name, a ceiling, a bad
artifact read. A fatal fault poisons the session: framing violations,
strict-JSON violations, id reuse, credit overdraw, handle desyncs. On a
...
race. The paths a release frame never travels are reclaimed host-side: a
call that minted handles and then settled `err` or `cancelled` reclaims
them, and goodbye, disconnect, and fatal faults reclaim everything.
