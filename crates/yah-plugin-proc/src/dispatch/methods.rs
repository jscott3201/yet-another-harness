...
impl WorkerMethodRequest {
    pub(super) fn new(payload: serde_json::Value) -> Self {
        Self { payload }
    }
...
/// concurrency. Implementations must return; cancellation can retire the
/// worker and its reply path, but cannot interrupt a synchronous callback.
...
fn is_reserved(name: &str) -> bool {
    RESERVED_EXACT.contains(&name) || name.starts_with("capability.")
}
