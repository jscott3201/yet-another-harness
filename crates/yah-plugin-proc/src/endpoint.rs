...
//!
//! Errors are typed, never parsed from prose. `LostCancelled` is the
//! session's no-reconciliation classification for a worker goodbye or a frame
//! proven never delivered. Deadline, bare disconnect/outcome-unknown, and
//! protocol-fatal loss remain conservative as their variants specify.
...
pub enum CallTerminal {
    ...
    /// never reached it. No reconciliation is required before a
    /// caller-policy retry.
    LostCancelled,
    /// Bare disconnect mid-call. The worker may have acted: reconcile
    /// external state first.
    LostOutcomeUnknown,
    /// The session named a protocol fault while the call was in flight.
    /// The kind is the closed wire vocabulary; reconciliation remains the
    /// caller's responsibility because the worker may have acted first.
    LostProtocolFault { kind: WireErrorKind },
}
    ...
    fn from(end: crate::CallEnd) -> Self {
        match end {
            ...
            crate::CallEnd::Lost { error, reconcile } => match (error, reconcile) {
                ...
                (WireErrorKind::OutcomeUnknown, _) => CallTerminal::LostOutcomeUnknown,
                (kind, _) => CallTerminal::LostProtocolFault { kind },
            },
        }
    }
}
...
impl ActivationEndpoint {
    ...
    ///
    /// Use [`ActivationEndpoint::call_stream`] to receive the worker's
    /// framed items.
    pub async fn call(
        &self,
        method: &str,
...
impl<'e> ArtifactReader<'e> {
    ...
    /// has no internal deadline. Activation end terminates it; callers
    /// needing a shorter bound must wrap the future in their own timeout.
