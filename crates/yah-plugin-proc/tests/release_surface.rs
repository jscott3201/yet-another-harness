...
/// A release-ack naming a kind the release did not carry is the same
/// desync it has always been: the session faults, and the waiter settles
/// as a bare loss — never as acknowledged success.
#[tokio::test]
async fn a_wrong_kind_ack_stays_a_fatal_protocol_fault() {
    release_script!(
        "release-kind",
        "release-ack-wrong-kind",
        driver,
        observer,
        rig,
        activation,
    ...
    .await
    .expect("the waiter settles when the session faults");
    assert_eq!(result, Err(EndpointError::ReleaseLost { orderly: false }));
    // The fault is named in health, not swallowed.
    let health = rig::health_becomes(&handle, "unhealthy", |health| {
        matches!(health, yah_plugin_host::PluginHealth::Unhealthy { .. })
    })
    .await;
    let yah_plugin_host::PluginHealth::Unhealthy { summary } = health else {
        unreachable!()
    };
    assert!(
        summary.contains("unknown-handle"),
        "health names the protocol fault: {summary}"
    );
    if let Some(gauges) = observer.gauges(&id) {
        assert_eq!(gauges.pending_releases, 0);
    }
