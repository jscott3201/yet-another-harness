;; Fixture guest: declares more tables than the host allows.
;;
;; The table twin of `many-memories.wat`, and it exists for the same reason: a
;; ceiling on table *elements* does not bound a table *count*. Every spare table
...
(component
  (core module $impl
    ...
    (table $spare1 0 funcref)
    (table $spare2 0 funcref)
    (table $spare3 0 funcref)
    (table $spare4 0 funcref)
    (table $spare5 0 funcref)
    (table $spare6 0 funcref)
    (table $spare7 0 funcref)
    (table $spare8 0 funcref)
    ...
    ;; Reached only if the count ceiling admitted every table above.
