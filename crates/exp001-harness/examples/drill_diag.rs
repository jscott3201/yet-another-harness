...
fn main() {
    ...
    let verdict: serde_json::Value = serde_json::from_slice(
        &std::fs::read(std::env::args().nth(2).expect("verdict path")).unwrap(),
    )
    .unwrap();
        ...
        match variant % 3 {
            ...
            _ => {
                ...
                if let Some(f) = filler
                    && f.len() >= 48
                {
                    break (f.as_bytes()[..48].to_vec(), payload.len() + 60);
                }
            }
        }
    };
    ...
    println!(
        "needle at {hit}, back_off {back_off}, target {target}, byte now {:#04x}",
        bytes[target]
    );
